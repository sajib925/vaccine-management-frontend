...
const LoginForm: React.FC = () => {
  ...
  const loginMutation = useMutation(
      async (formData: FormData) => {
        const res = await axios.post(
            "https://vaccine-management-supebase.vercel.app/api/auth/login/",
            formData,
            {
              headers: {
  ...
  const handleDoctorLogin = () => {
    setValue("username", "doctor");
    setValue("password", "123456a@");
  };

  const handlePatientLogin = () => {
    setValue("username", "patient");
    setValue("password", "123456a@");
  };
  ...
  return (
      <div className="max-w-screen-lg w-full mx-auto my-32 lg:my-60 px-5">
        <div className="grid grid-cols-1 lg:grid-cols-2 items-center gap-y-4 gap-x-20">
          ...
          <Card>
            ...
            <CardFooter className="flex items-center justify-center gap-2 pt-0">
              ...
              <Link
                  href="/signUp"
                  className="text-blue-900 font-semibold hover:underline hover:text-black transition-all ease-in-out cursor-pointer"
              >
                Register Now
              </Link>
            </CardFooter>
        ...
        <Card className="mt-5 max-w-[400px] w-full mx-auto flex flex-col items-center justify-center">
          ...
          <CardDescription className="px-2 pb-4 text-center text-lg text-gray-700">I have two types of roles Doctor and Patient You can access what type you need by clicking the button below</CardDescription>
          <div className="flex items-center justify-center gap-4 pb-4">
            <Button onClick={handleDoctorLogin}>Doctor</Button>
            <Button onClick={handlePatientLogin}>Patient</Button>
          </div>
        </Card>
      </div>
  );
